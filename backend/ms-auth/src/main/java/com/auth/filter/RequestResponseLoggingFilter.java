...
/**
 * Reactive Request and Response Logging Filter for ms-auth.
 * 
 * This service uses WebFlux (Netty), NOT Servlet (Tomcat).
 * Therefore, we use WebFilter instead of OncePerRequestFilter (Servlet).
 * 
 * Features:
...
public class RequestResponseLoggingFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        // Step 3: Log request
        logRequest(method, fullUrl, clientIp);

        // Step 4: Execute the request and log response
        return chain.filter(exchange).then(
            Mono.fromRunnable(() -> {
                ...
                logResponse(method, fullUrl, clientIp, status, duration);
            })
        );
    }

    /**
     * Logs the incoming request with colored output.
     */
    private void logRequest(String method, String fullUrl, String clientIp) {
        String methodColor = METHOD_COLORS.getOrDefault(method, ANSI_RESET);
        String coloredMethod = colorize(String.format("%-7s", method), methodColor);
        String coloredUrl = colorize(fullUrl, ANSI_WHITE);
        String coloredIp = colorize("[" + clientIp + "]", ANSI_BLUE);

        log.info("{} {} - {}", coloredMethod, coloredUrl, coloredIp);
    }

    /**
     * Logs the response with colored status and duration.
     */
    private void logResponse(String method, String fullUrl, String clientIp, int status, long duration) {
        String statusColor;
        String statusLabel;
        if (status >= 500) {
        ...
        String methodColor = METHOD_COLORS.getOrDefault(method, ANSI_RESET);
        String paddedMethod = colorize(String.format("%-7s", method), methodColor);
        ...
        if (status >= 500) {
            log.error("{} {} {} - {} {} [{}]",
                paddedMethod, coloredStatus, coloredUrl, coloredIp, coloredDuration, statusLabel);
        } else if (status >= 400) {
            log.warn("{} {} {} - {} {} [{}]",
                paddedMethod, coloredStatus, coloredUrl, coloredIp, coloredDuration, statusLabel);
        } else {
            log.info("{} {} {} - {} {}",
                paddedMethod, coloredStatus, coloredUrl, coloredIp, coloredDuration);
        }
    }
