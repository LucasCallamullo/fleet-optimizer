...
public class RequestResponseLoggingFilter extends OncePerRequestFilter {
    ...
    // HTTP Method Bold Color Mapping for Method Names
    /* private static final Map<String, String> METHOD_BOLD_COLORS = Map.of(
            "GET", ANSI_BOLD_GREEN,
            "POST", ANSI_BOLD_CYAN,
            "PUT", ANSI_BOLD_YELLOW,
            "PATCH", "\u001B[1;35m", // Bold Magenta
            "DELETE", ANSI_BOLD_RED,
            "OPTIONS", "\u001B[1;34m", // Bold Blue
            "HEAD", "\u001B[1;37m" // Bold White
    );   */
    ...
    /**
     * Get the request body from the wrapped request
     * 
     * @param request The HTTP request
    ...
    private String getRequestBody(HttpServletRequest request) {
        try {
            if (request instanceof ContentCachingRequestWrapper) {
                ...
                return new String(body, wrapper.getCharacterEncoding());
            }
        } catch (Exception e) {
            log.debug("Unable to read request body: {}", e.getMessage());
        }
        return "Unable to read body";
    } */

    /**
     * Get the response body from the wrapped response
     * 
    ...
    private String getResponseBody(HttpServletResponse response) {
        try {
            ...
            log.debug("Unable to read response body: {}", e.getMessage());
        }
        return "Unable to read body";
    } */
